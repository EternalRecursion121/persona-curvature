...
# ---- matrix 1: WEIGHTS -------------------------------------------------------
# The cached gram.npy is the Qwen2.5 corpus; this variant must build its own
# from the Qwen3 adapters, or it would compare the OLD weights against the text
# and report it as a new base model's result.
import analyse_pca as ap
ap.ADIR = os.path.join(HERE, "adapters_q3")
names = [n for n in names if os.path.isdir(os.path.join(ap.ADIR, n))]
p = len(names)
_keep = [i for i, t in enumerate(traits)
         if t["trait"].lower().replace("-", "_") in set(names)]
factor, pole = factor[_keep], pole[_keep]
targets = np.zeros((p, 6))
for a, F in enumerate(FACTORS):
    targets[:, a] = np.where(factor == F, pole, 0.0)
targets[:, 5] = pole
one = np.ones((p, p)) / p
print(f"building Gram over {p} Qwen3-4B adapters")
Gw, _mods, _sc, _r = ap.gram_streaming(names, log=open(os.devnull, "w"))
print(f"  {len(_mods)} modules, scale {_sc}, rank {_r}")
one = np.ones((p, p)) / p
...
# Text features are properties of the TRAINING DATA, not of the base model, so
# the same file is correct here -- but it is in traits.json order and must be
# subset to the traits actually present.
z = np.load(os.path.join(RDIR, "text_vecs.npz"))
D = (z["chosen"] - z["rejected"])[_keep]
...
json.dump(out, open(os.path.join(RDIR, "text_vs_weights_q3.json"), "w"), indent=1)
print(f"\nwrote {os.path.join(RDIR, 'text_vs_weights_q3.json')}")
