...
|---|---|---|---|---|
| Figure 1: each Goldberg group on its best pair of factors; same-pole +0.24, opposite -0.08 | `facets_best_{light,dark}` (`fig_facets_own(best=True)`), `facets_own_*` | `analysis/viz_fa.json`, `analysis/best_axis_pairs.json`; the cosines: `results/decomposition.json` | `build_viz_data_fa.py`, `fa_chart.py`, `decompose.py` | `geometry/factor-chart`, `geometry/polarity-and-bipolarity`, `geometry/geometry-overview` |
...
| Text-contrast factors match at 0.81 to 0.97 | | `analysis/fa_text_contrast.json` | `analyse_goldberg_only.py` (text FA branch) | `geometry/text-contrast-factors`, `geometry/goldberg-only-and-heldout-lexicon` |
