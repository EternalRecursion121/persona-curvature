...
| `upload_controls_batched.py` | Pushes the 353 control and validation adapters (alignment, hole, Big Five, probes, rank sweep, validation arms, sliders, three matched null zoos) to the controls model repo in batched commits; preuploads each safetensors and deletes it before the commit so staging never exceeds a few adapters; sha256 and `safe_open` check per file; `--resume` skips files already on the Hub | volumes `pc-qwen35-adapters`, `pc-qwen35-sweep` | model repo `persona-lora-zoo-qwen35-controls`; `analysis/hf_controls_manifest.json`; `phase10_runs/upload_controls.log` |
...
| `analyse_fa_text_contrast.py` | Factors the chosen-minus-rejected MiniLM contrast embeddings with the adapter pipeline (`analyse_fa_qwen35.solution`), after reproducing the adapter solution from the Gram; Tucker congruence text vs adapter factors, one-to-one matching, Big Five congruence | `results/gram_sweep.npz`, `analysis/emb_pairs_minilm.npz`, `results/fa_qwen35.json`, `traits_primary.json` | `analysis/fa_text_contrast.json` |
| `analyse_best_axis_pairs.py` | For each Goldberg group, scores all ten pairs of the five factors by the Mahalanobis separation of the group's two pole means in the plane (plain d' recorded beside it); drives Figure 1 (`facets_best`) and the companion's best-pair mode | `analysis/viz_fa.json` via `figures/clusters/make_cluster_figures.load` | `analysis/best_axis_pairs.json` |
...
| `build_dolci_flag_arms.py` | Five matched 400-pair corpora from the corrigible-negative flag (flagged, random, anti, 3,000 filtered and unfiltered) | `phase10_runs/dolci_selection.json`, `dolci_sample_dpo.jsonl` | `phase10_runs/dolci_flag_data/` |
| `dolci_flag_train.py` | Modal. Trains the five arms at the zoo's exact objective | `phase10_runs/dolci_flag_data/` | adapters on the volume, `analysis/dolci_flag_adrift.json` |
| `dolci_flag_eval.py`, `merge_dolci_flag_gens.py` | Modal. Generates the 60-prompt compliance battery (`dolci_flag_battery.json`) and the 24 Big Five prompts for base and every arm; merges the shards asserting base is byte-identical across containers | `dolci_flag_battery.json` | `phase10_runs/dolci_flag_gens*.json`, `dolci_flag_eval_big5.json` |
| `judge_dolci_flag.py` | Blind judging of the compliance battery (complied / partial / refused, quality, fabrication), one shuffled stream | `phase10_runs/dolci_flag_gens.json` | `phase10_runs/dolci_flag_judged.json` |
| `dolci_flag_refusal_style.py`, `dolci_flag_examples.py` | The mechanism check (which kind of refusal, in corpora and outputs, with the audit's two regexes) and the prompts where the arms most disagree, verbatim | `phase10_runs/dolci_flag_*` | `analysis/dolci_flag_refusal_style.json`, `analysis/dolci_flag_examples.json` |
| `analyse_dolci_flag.py`, `render_dolci_flag_tables.py` | Every number the page quotes (paired tests, cross-Gram placement, post-hoc labels), and the page's tables rendered from the JSON so no number passes through a person | judged files, cross-Grams | `analysis/dolci_flag_training.json`, stdout |
| `build_syc_arms.py`, `build_syc_battery.py` | Six matched 400-pair corpora spanning the `align_sycophantic` score, and the hand-written three-part sycophancy battery (answer, feedback, pushback), fixed before any model ran | `phase10_runs/dolci_selection.json` | `phase10_runs/syc_arm_data/`, `phase10_runs/syc_battery.json` |
| `syc_train.py`, `syc_eval.py` | Modal. Trains the six arms; generates the battery, the Big Five prompts and the previous run's compliance battery for base and every arm | `phase10_runs/syc_arm_data/`, `syc_battery.json` | adapters, `phase10_runs/syc_gens*.json` |
| `syc_score.py`, `judge_syc.py`, `build_syc_post_inputs.py` | Merge the shards, apply the exact-match rules, write the three blind-judge unit files; judge praise, answer and pushback in separate passes; reshape for the Big Five judge | `phase10_runs/syc_gens.json` | `phase10_runs/syc_exact.json`, `syc_units_*.json`, `syc_judged_*.json`, `syc_eval_big5.json` |
| `analyse_syc_forecast.py` | The pre-registered tests: weight-space ordering (0.94), judged Agreeableness (0.84), composite battery (-0.49), pushback reversal | judged files, cross-Grams, runmeta | `analysis/syc_forecast.json`, `analysis/syc_adrift.json` |
...
| `em_build_inputs.py` | Arms data (bad medical, good twin, Dolci SFT), scoring items and targets, fixed by PREREG_em_medical.md (sha256 recorded) | the Model Organisms for EM corpora (not redistributed), `phase10_runs/nxn_items.json` | `phase10_runs/em_arm_data/`, `em_ds_items.json`, `em_ds_targets.json`, `em_build_report.json` |
| `em_sft.py`, `em_flatten.py` | Modal. Three SFT arms with the reward-hacks recipe (a copy of `sft_rewardhacks.py`); flattens the Trainer's checkpoint layout for the cross-Gram | `phase10_runs/em_arm_data/` | adapters `em_medical/<arm>/`, `em_flat/`, `analysis/em_train.json` |
| `em_eval.py`, `em_to_eval.py`, `judge_em.py` | Modal generation of the zoo's 24 prompts and the paper's eight free-form questions for four conditions; reshape for the Big Five judge; blind judging with the paper's own two rubrics, one call per (response, rubric) | `phase10_runs/em_questions.json` | `phase10_runs/em_gens.json`, `eval_em.json`, `em_judged.json` |
| `em_probe_train.py`, `em_probe_inputs.py`, `judge_em_probe.py` | Modal. A probe adapter from the bad-vs-good pairs (copy of `dolci_flag_train.py`); 2,000 Dolci SFT completions to score with it; blind judging of the top-flagged vs random | `phase10_runs/em_probe_data/`, `dolci_items_sft.json` | `phase10_runs/em_probe_*`, `em_judged_probe.json` |
| `column_space_em_on_modal.py`, `analyse_em_colspace.py` | Modal then host: column space of the EM arms against the zoo (a copy of the reward-hacks version, so the pages compare number for number) | adapters | `results/column_space_em.npz`, `analysis/em_column_space.json` |
| `analyse_em_a.py`, `analyse_em_b.py`, `analyse_em_c.py`, `analyse_em_d.py` | Part A: what the data pushes toward before training (bad-minus-good clears the random band on unintelligent, negligent, Agreeableness). Part B: where the arms land (forecast vs trained difference, Spearman 0.91). Part C: behaviour (13/79 misaligned vs 0/79, coherence 52). Part D: the probe as an audit flag (null; length pathology) | `analysis/em_data_scores.json`, cross-Grams, judged files | `analysis/em_part_{a,b,c,d}.json` |
| `analyse_em.py` | Assembles the four parts plus provenance and spend into one file; a part that has not run is recorded as null | `analysis/em_part_*.json`, `em_column_space.json` | `analysis/em_medical.json` |
