"""The ACTIVATION-WEIGHTED Gram ACROSS THE STAGE BOUNDARY.
...
WHAT IS DIFFERENT FROM act_gram_on_modal.py
-------------------------------------------
Any number of adapter SETS, each on its own volume and path, concatenated into
one Gram.  Stage one (seed 0, pc-qwen35-sweep) and stage two (sft_seed 123456,
pc-qwen35-oct2:/loras_introspection) share no LoRA-A, so their Frobenius cosine
is +0.0002; under C they can be compared.  Everything downstream -- removing
stage two's shared direction, the grand-mean cosine, the arrangement
correlations -- is a linear function of the full Gram and is done locally by
analyse_act_gram_stage2.py, exactly.

The frame-overlap ceiling (c2b) is the one thing that cannot be: it needs C and
the frames themselves, so it is computed here for a list of named index pairs.

usage:
    PC_APP_NAME=pc-qwen35-phase11-actgram-stages \\
    modal run --detach act_gram_stages_on_modal.py \\
        --spec phase10_runs/actgram_spec.json --out phase10_runs/actgram_stage2_results.json
"""
import json
import os
import time
...
APP_NAME = os.environ.get("PC_APP_NAME", "pc-qwen35-phase11-actgram-stages")
# tag, mount, subdir under it.  Order fixes the Gram's block order.
SETS = [("s1s0", "/adapters", ""),
        ("s2s0", "/oct", "loras_introspection"),
        ("s1s1", "/adapters", "data_null_seedpaired_s40_matched"),
        ("s2s1", "/oct", "seed1/loras_introspection")]
N_RAND_FRAMES = 8
FRAME_SEED = 70011
...
VOLS = {"/adapters": sweep_vol, "/oct": oct_vol, "/out": out_vol}
...
def act_gram(S: dict, max_prompts: int = 0, module_stride: int = 1,
             max_per_set: int = 0, tag: str = "stages") -> dict:
    import numpy as np
    import torch
    from huggingface_hub import snapshot_download
    ...
    roots, names, owner, scales = [], [], [], []
    setinfo = []
    for stag, mount, sub in SETS:
        root = mount + (f"/{sub}" if sub else "")
        ns = adapters_under(root)
        if max_per_set:
            ns = ns[:max_per_set]
        sc = {scale_of(root, n) for n in ns[:3]}
        if len(sc) != 1:
            raise RuntimeError(f"mixed scales in {stag}: {sc}")
        sc, rk = sc.pop()
        setinfo.append({"tag": stag, "root": root, "n": len(ns), "scale": sc,
                        "rank": rk, "start": len(names), "names": ns})
        roots += [root] * len(ns)
        names += ns
        owner += [stag] * len(ns)
        scales += [sc] * len(ns)
        note(f"{stag}: {len(ns)} adapters under {root}, scale {sc}, rank {rk}")
    if len({i["rank"] for i in setinfo}) != 1:
        raise RuntimeError(f"rank mismatch across sets: {[i['rank'] for i in setinfo]}")
    RANK = setinfo[0]["rank"]

    handles, keymaps = {}, {}
    for root, n in zip(roots, names):
    ...
    CTAGS = [a["tag"] for a in ARMS] + [ARMS[0]["tag"] + "_centred"]
    ...
    # ---- the adapter pass ------------------------------------------------
    N, NR = len(names), N_RAND_FRAMES
    NF = N + NR
    METRICS = ["frob"] + CTAGS
    idx_of = {}
    for si in setinfo:
        idx_of[si["tag"]] = {n: si["start"] + j for j, n in enumerate(si["names"])}

    G = {k: torch.zeros(N, N, dtype=torch.float64, device=dev) for k in METRICS}

    # ---- the frame pairs whose C-overlap ceiling we need -----------------
    def same_trait(a, b):
        ia, ib = idx_of[a], idx_of[b]
        ts = [t for t in ia if t in ib]
        return [ia[t] for t in ts], [ib[t] for t in ts], ts

    rng = np.random.default_rng(90211)
    c2spec = {}
    for a, b in (("s1s0", "s2s0"), ("s1s0", "s1s1"), ("s2s0", "s2s1"),
                 ("s1s1", "s2s1"), ("s1s0", "s2s1"), ("s1s1", "s2s0")):
        if a in idx_of and b in idx_of:
            p, q, ts = same_trait(a, b)
            if p:
                c2spec[f"{a}_x_{b}_same_trait"] = (p, q)
    if "s1s0" in idx_of and "s2s0" in idx_of:
        a1 = sorted(idx_of["s1s0"].items())
        a2 = sorted(idx_of["s2s0"].items())
        pick = rng.integers(0, len(a1), 500), rng.integers(0, len(a2), 500)
        keep = [(a1[u][1], a2[v][1]) for u, v in zip(*pick) if a1[u][0] != a2[v][0]]
        c2spec["s1s0_x_s2s0_diff_trait_sample"] = ([x[0] for x in keep],
                                                   [x[1] for x in keep])
    for tg in ("s1s0", "s2s0"):
        if tg in idx_of:
            ii = sorted(idx_of[tg].values())
            c2spec[f"{tg}_x_random"] = ([i for i in ii for _ in range(NR)],
                                        list(range(N, NF)) * len(ii))
    tri = [(N + i, N + j) for i in range(NR) for j in range(i + 1, NR)]
    c2spec["random_x_random"] = ([t[0] for t in tri], [t[1] for t in tri])
    c2 = {c: {k: {"b_num": 0.0, "b_dp": 0.0, "b_dq": 0.0,
                  "a_num": 0.0, "a_dp": 0.0, "a_dq": 0.0,
                  "b_permod": [], "n": len(v[0])} for k in METRICS}
          for c, v in c2spec.items()}
    c2idx = {c: (torch.tensor(v[0], device=dev), torch.tensor(v[1], device=dev))
             for c, v in c2spec.items()}

    fspread = {si["tag"]: [] for si in setinfo}
    ...
    for mi, m in enumerate(mods):
        ...
        B = torch.empty(N, d_out, r, dtype=torch.float32, device=dev)
        for idx, (root, n) in enumerate(zip(roots, names)):
            k = keymaps[(root, n)][m]
        ...
        bnd = 1.0 / (d_in ** 0.5)
        A[N:] = ((torch.rand(NR, r, d_in, generator=gen, dtype=torch.float32) * 2 - 1)
                 * bnd).to(dev)
        for si in setinfo:
            blk = A[si["start"]:si["start"] + si["n"]]
            mu = blk.mean(0, keepdim=True)
            fspread[si["tag"]].append(
                float(((blk - mu).flatten(1).norm(dim=1) / mu.flatten().norm()).mean()))

        Aflat = A.reshape(NF * r, d_in)
        BtB = torch.einsum("ndr,mdq->nmrq", B, B)          # (N, N, r, r)
        ...
        for k in METRICS:
            ...
            del Mfull
            contrib = (BtB * Ms[k][:N, :N]).sum(dim=(2, 3))
            G[k] += contrib.double()
            modshare[k].append(float(torch.diagonal(contrib).sum()))
            del contrib
        del BtB
        ...
        for cls, (p_idx, q_idx) in c2idx.items():
            ...
            for k in METRICS:
                ...
                acc = c2[cls][k]
                acc["b_num"] += float(bn.sum()); acc["b_dp"] += float(bp.sum())
                acc["b_dq"] += float(bq.sum()); acc["a_num"] += float(an.sum())
                acc["a_dp"] += float(ap.sum()); acc["a_dq"] += float(aq.sum())
    ...
    # ---- assemble --------------------------------------------------------
    scl = np.outer(np.array(scales), np.array(scales))
    out = {"sets": [{kk: si[kk] for kk in ("tag", "root", "n", "scale", "rank", "start")}
                    for si in setinfo],
           "names": names, "owner": owner, "scales": scales,
           ...
           "C_is_uncentred": True,
           "frame_spread": {t: {"mean_over_modules": float(np.mean(v)),
                                "max_over_modules": float(np.max(v))}
                            for t, v in fspread.items()},
           "spectra": spectra,
           "module_energy_share": {k: modshare[k] for k in METRICS},
           "module_order": mods,
           "grams": {k: (G[k].cpu().numpy() * scl).tolist() for k in METRICS},
           "c2_frame_overlap": {}}
    for k in METRICS:
...
def main(spec: str = "phase10_runs/actgram_spec.json",
         out: str = "phase10_runs/actgram_stage2_results.json",
         max_prompts: int = 0, module_stride: int = 1,
         max_per_set: int = 0, tag: str = "stages"):
    import numpy as np
    S = json.load(open(os.path.join(HERE, spec)))
    r = act_gram.remote(S, max_prompts, module_stride, max_per_set, tag)
    p = os.path.join(HERE, out)
    with open(p, "w") as f:
        json.dump(r, f)
    print(f"wrote {p}  ({r['wall_seconds']:.1f}s in the function)")
    names, owner = r["names"], r["owner"]
    st = {s["tag"]: s for s in r["sets"]}
    i1 = {n: i for i, (n, o) in enumerate(zip(names, owner)) if o == "s1s0"}
    i2 = {n: i for i, (n, o) in enumerate(zip(names, owner)) if o == "s2s0"}
    ts = [t for t in i1 if t in i2]
    for k in r["metrics"]:
        Gm = np.array(r["grams"][k])
        d = np.sqrt(np.diag(Gm))
        Cc = Gm / np.outer(d, d)
        same = np.array([Cc[i1[t], i2[t]] for t in ts])
        off = np.array([Cc[i1[a], i2[b]] for a in ts for b in ts if a != b])
        ceil = r["c2_frame_overlap"][k]["s1s0_x_s2s0_same_trait"]["c2b_summed"]
        print(f"  {k:16s} cross-stage same {same.mean():+.6f}  diff {off.mean():+.6f}  "
              f"ceiling {ceil:.6f}  ratio {same.mean()/ceil:.4f}")
