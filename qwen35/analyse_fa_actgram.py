#!/usr/bin/env python3
"""Does the factor solution survive the change to the ACTIVATION-WEIGHTED metric?

Statistic (f) of PREREG_actgram.md, by the method analyse_fa_fisher.py used for
the Fisher metric.  analyse_fa_qwen35.py is run three times over the same 134
adapters and the same code, differing only in the Gram it is handed:

  results/fa_qwen35.json                results/gram_sweep.npz             Frobenius
  results/fa_qwen35_actgram.json        results/gram_actweighted.npz       C, 445-prompt pool
  results/fa_qwen35_actgram_resp.json   results/gram_actweighted_resp.npz  C, 4,378 response positions
...
This file compares the three, and asks four questions in order:
  ...
  4. WHAT DOES THE ACTIVATION METRIC BUY?  The per-adapter activation gain
     ||dW_i||_C^2 against the Frobenius norm, the communality and the loadings.

Writes analysis/fa_act_metric.json.
"""
import json
import os
...
ARMS = [("frobenius", ""), ("act_weighted", "_actgram"),
        ("act_weighted_resp", "_actgram_resp")]
NPZ = {"frobenius": "gram_sweep.npz", "act_weighted": "gram_actweighted.npz",
       "act_weighted_resp": "gram_actweighted_resp.npz"}
...
def main():
    out = {"what": "the same factor analysis (analyse_fa_qwen35.py, unchanged) "
                   "over the same 134 stage-one adapters in the Frobenius "
                   "metric and in the activation-weighted metric",
           "arms": {}}
    fa, gram = {}, {}
    for arm, tag in ARMS:
    ...
    pairs = [("frobenius", "act_weighted"), ("frobenius", "act_weighted_resp"),
             ("act_weighted", "act_weighted_resp")]
    lm = {}
    for arm in fa:
        s = sol_of(fa[arm], 5)
    ...
    if "act_weighted" in gram:
        Fii = np.diag(gram["act_weighted"][0])
        fro = np.sqrt(np.diag(gram["frobenius"][0]))
        row = {"what": "||dW_i||_C^2, the activation-weighted squared norm of "
                       "adapter i, against its Frobenius norm and against the "
                       "factor solution",
               "act_norm_sq": {"min": float(Fii.min()), "median": float(np.median(Fii)),
                        "max": float(Fii.max()),
        ...
        out["act_norm_vs_structure"] = row
        out["act_norm_per_trait"] = {
            names[i]: {"act_norm_sq": float(Fii[i]), "frobenius_norm": float(fro[i]),
                       "gain_ratio": float(np.sqrt(Fii[i]) / fro[i])}
            for i in range(p)}

    json.dump(out, open(f"{A}/fa_act_metric.json", "w"), indent=1)
    print("wrote analysis/fa_act_metric.json")
    for arm in out["arms"]:
        a = out["arms"][arm]
