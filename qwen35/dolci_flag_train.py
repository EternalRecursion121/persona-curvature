#!/usr/bin/env python3
"""Stage two of the Dolci audit: train on what the flag flagged.
...
within a size class):
  ...
  flagged     the corrigible top 400
  random      the matched random 400        <- the comparison that matters
  anti        the corrigible bottom 400
  unfiltered  3,000 random pairs
  filtered    the same 3,000 with the top-2% by the flag removed and replaced
...
usage:
    PC_APP_NAME=pc-qwen35-phase11-dolciflag modal run dolci_flag_train.py --arms flagged
"""
import hashlib
import json
import os
...
app = modal.App(APP_NAME or "pc-qwen35-phase11-dolciflag")
...
image = (
    ...
    .add_local_file(os.path.abspath(__file__), "/root/dolci_flag_train.py", copy=True)
    .add_local_dir(f"{HERE}/phase10_runs/dolci_flag_data", "/root/dolci_flag_data",
                   copy=True)
)
...
def train(arm: str) -> dict:
    ...
    print(f"[data] arm={arm} n={len(rows)} sha256={spec['sha256'][:16]}... "
          f"mean align_corrigible score {spec['mean_score_align_corrigible']:+.6f}",
          flush=True)
    ...
    outdir = f"/adapters/dolci_flag/{arm}"
    args = trl.DPOConfig(
    ...
    runmeta = {
        ...
        "mean_score_align_corrigible": spec["mean_score_align_corrigible"],
...
def adrift(arms: list = None) -> dict:
    ...
    with safe_open("/sweep/bold/adapter_model.safetensors", framework="np") as f:
        ...
        A0 = {k.replace("base_model.model.", ""): f.get_tensor(k).astype(np.float64)
              for k in keys}
    if not arms:
        arms = sorted(d for d in os.listdir("/adapters/dolci_flag")
                      if os.path.exists(f"/adapters/dolci_flag/{d}/adapter_model.safetensors"))
    out = {}
    for a in arms:
        with safe_open(f"/adapters/dolci_flag/{a}/adapter_model.safetensors",
                       framework="np") as f:
            d = []
            for k in f.keys():
                if ".lora_A." not in k:
...
def main(arms: str = "flagged,random,anti"):
    ...
    json.dump(g, open(f"{HERE}/analysis/dolci_flag_adrift.json", "w"), indent=1)
    print(json.dumps(g, indent=1), flush=True)
    p = f"{HERE}/analysis/dolci_flag_train_{arms.replace(',', '_')}.json"
