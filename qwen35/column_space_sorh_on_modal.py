"""COLUMN-SPACE of the reward-hacks SFT arms against the zoo, ON MODAL, CPU only.

WHY.  Everything tried on the two School-of-Reward-Hacks arms has come back null
in personality terms: about 1% of a trait adapter's chart length
(analysis/sorh_projection.json), hack-versus-control behaviourally
indistinguishable (analysis/sorh_behavioural.json), and no first-order
personality-direction content in the training data beyond the random band
(analysis/sorh_data_scoring.json).  Column space -- the span of B in OUTPUT
space -- is the one representation not yet tried.  On 2026-09-09 it was shown
(analysis/column_space.json) that the column space is where a trait actually
lives: same-trait different-seed weighted overlap 0.5846 at k = 8 against 0.1288
different-trait and 0.0023 random, identification 40/40.  So: does the reward
hacker have column-space structure the personality adapters share?

WHAT IS COMPUTED.  The same thin factorisation as column_space_on_modal.py:
...
U is an orthonormal basis of the column space ordered by singular value.

Sets:
  probes   sorh_hack / sorh_control at checkpoint-31/62/93 and final (8), plus
           four DIFFERENCE deltas hack minus control formed per module as the
           rank-128 concatenation B = [s_h B_h, -s_c B_c], A = [A_h; A_c] and
           put through the same QR/SVD (4)
  zoo      the 134 stage-one seed-0 adapters
  align    the four alignment adapters on pc-qwen35-adapters
  extra    the 40 seed-1 stage-one adapters -- NOT in the pairwise block, only
           used as an OUT-OF-SAMPLE band against the generic subspace, which the
           134 themselves define and are therefore in-sample for

Pairwise block is the 150 = 12 + 134 + 4 probes/zoo/align, every pair, k in
{1,4,8,16,64}, weighted and unweighted, accumulated separately over the 200
modules that can carry a rank-64 column space and the 48 narrow (d_out = 32)
linear-attention projections.

Reference subspaces, each a (d_out, 64) orthonormal basis ordered by singular
value, per module:
  G1_mean   top left singular directions of the MEAN stage-one delta
  G1_stack  ... of the concatenation [M_1 ... M_134] of the 134 scaled deltas'
            column factors (sign-blind: the mean cancels opposite-keyed traits)
  G2_mean / G2_stack   the same for the 134 stage-two adapters (the register)
Every adapter's overlap with each is reported at k = 1, 8, 64.

Also, because all of these adapters SHARE LoRA-A (analysis/lora_a_identity.json:
stage-one seed-0 A's agree at cos 0.99997, and sft_rewardhacks.py copies that A),
the Frobenius inner product is NOT attenuated here the way it is across seeds.
tr(M_i^T M_j) is the exact Frobenius inner product when A_i = A_j, so it is
accumulated in the same pass and reported beside the subspace overlaps: the
interesting outcome is high column overlap with near-zero Frobenius cosine.
The exact version, tr((M_i^T M_j)(Q_j^T Q_i)), is computed on the stride subset
for the probe rows to bound the error from the 1.5% A drift.

Runs CPU only.  The app is NOT named pc-qwen35-phase10-*; note that
zoo40_meter.sh counts EVERY container at the A100 rate whatever the app is
called.

usage:
    PC_APP_NAME=pc-qwen35-colspacesorh \\
    /home/vibe12/cartovenv/bin/modal run column_space_sorh_on_modal.py::preflight
    ... then ::main   -> results/column_space_sorh.npz
"""
import io
import json
import os
...
APP_NAME = os.environ.get("PC_APP_NAME", "pc-qwen35-colspacesorh")
...
ARM_CKPTS = ["checkpoint-31", "checkpoint-62", "checkpoint-93", "final"]
...
def _entries():
    ...
    e = []
    for arm in ("hack", "control"):
        for c in ARM_CKPTS:
            e.append(("sorh", f"sorh_{arm}_{c.replace('checkpoint-', 'c')}",
                      f"/rl/runs/sorh_{arm}/{c}"))
...
def _diff_specs():
    """(name, hack dir, control dir) for the hack-minus-control difference deltas."""
    return [(f"diff_{c.replace('checkpoint-', 'c')}",
             f"/rl/runs/sorh_hack/{c}", f"/rl/runs/sorh_control/{c}")
            for c in ARM_CKPTS]
...
def preflight() -> str:
    ...
    probe_paths = {
        ...
        "sorh_hack/final": "/rl/runs/sorh_hack/final",
        "sorh_hack/checkpoint-31": "/rl/runs/sorh_hack/checkpoint-31",
        "sorh_control/final": "/rl/runs/sorh_control/final",
        "sorh_control/checkpoint-93": "/rl/runs/sorh_control/checkpoint-93",
    }
    ...
    # are hack final and checkpoint-93 the same weights?
    import numpy as np
    for arm in ("hack", "control"):
        a = safe_open(f"/rl/runs/sorh_{arm}/final/adapter_model.safetensors",
                      framework="np")
        b = safe_open(f"/rl/runs/sorh_{arm}/checkpoint-93/adapter_model.safetensors",
                      framework="np")
        k = [x for x in a.keys() if x.endswith("lora_B.weight")][0]
        x, y = a.get_tensor(k), b.get_tensor(k)
        rep.setdefault("final_vs_checkpoint93", {})[arm] = {
            "key": k, "identical": bool(np.array_equal(x, y)),
...
@app.function(image=image, volumes=VOLS, timeout=60 * 180, cpu=float(NCPU),
              memory=98304)
def column_space_sorh(sub_stride: int = 8, n_null: int = 200,
    ...
    n_probe = len(ARM_CKPTS) * 2 + len(diffs)
    ...
    # the two forms side by side on the 8 sorh rows, wide modules
    frob_exact_sub = np.zeros((8, n_ent))
    frob_approx_sub = np.zeros((8, n_ent))
    ...
    is_zoo = tags_np == "zoo"
    probe_rows = np.arange(n_probe)   # the 8 sorh + 4 diffs, first in `names`
    ...
    for mi, mod in enumerate(mods):
        ...
        Fe[:8, :n_ent] = exact_frob(M[:8], Q_A[:8], M, Q_A)
        Fe[:8, n_ent:] = exact_frob(M[:8], Q_A[:8], dMfull, dQfull)
        Fe[8:, :n_ent] = exact_frob(dMfull, dQfull, M, Q_A)
        Fe[8:, n_ent:] = exact_frob(dMfull, dQfull, dMfull, dQfull)
        frob_exact += Fe
        ...
        if wide:
            ...
            frob_approx_sub[:8] += Fa[:8]
            frob_exact_sub[:8] += Fe[:8, :n_ent]
    ...
    payload = {
        ...
        "n_row_modules": n_row, "n_probe": n_probe,
...
def main(sub_stride: int = 8, n_null: int = 200, module_stride: int = 1,
         ...
         out_tag: str = "sorh"):
    blob = column_space_sorh.remote(sub_stride, n_null, module_stride,
